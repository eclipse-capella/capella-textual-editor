...
public class EmbeddedEditorInstanceHelper2 {
  ...
  public static List<AbstractEvent> getAvailableExchanges(String source, String target) {
    ...
    InstanceRole sourceIr = EmbeddedEditorInstanceHelper2.getInstanceRole(source);
    InstanceRole targetIr = EmbeddedEditorInstanceHelper2.getInstanceRole(target);
    Scenario currentScenario = EmbeddedEditorInstance.getAssociatedScenarioDiagram();
  ...
  /**
   * returns the list of already inserted elements as names
   * 
   * @param keyword
   * @return list of already existing participants names in diagram
   *
   */
  public static List<String> getInsertedParticipantsNames(String keyword) {
    Scenario diagram = EmbeddedEditorInstance.getAssociatedScenarioDiagram();
    List<String> existingNames = new ArrayList();
    diagram.getOwnedInstanceRoles().forEach(x -> {
      AbstractType irType = x.getRepresentedInstance().getAbstractType();
      
      switch (keyword) {
      case DslConstants.ENTITY:
        if(irType instanceof Entity)
          existingNames.add(x.getName());
        break;
      case DslConstants.CONFIGURATION_ITEM:
        if(irType instanceof ConfigurationItem)
          existingNames.add(x.getName());
        break;
      case DslConstants.ACTOR:
        if(irType instanceof Component && ((Component) irType).isActor())
          existingNames.add(x.getName());
        break;
      case DslConstants.COMPONENT:
        if(irType instanceof Component)
          existingNames.add(x.getName());
        break;
      case DslConstants.ACTIVITY:
        if(x.getRepresentedInstance() instanceof OperationalActivity)
          existingNames.add(x.getName());
      case DslConstants.ROLE:
        if(x.getRepresentedInstance() instanceof Role)
          existingNames.add(x.getName());
      case DslConstants.FUNCTION:
        if(x.getRepresentedInstance() instanceof AbstractFunction)
          existingNames.add(x.getName());
        break;
      default:
        break;
