...
/**
 * !!! This class was copied from Capella
 * (org.polarsys.capella.core.data.interaction.properties.controllers.DataFlowHelper) to support searching of available
 * exchanges between the elements (not only between instance roles); Example of refactoring in
 * getAvailableComponentExchanges(InstanceRole, InstanceRole) will call getAvailableComponentExchanges(Component, Component)
 */
public class DataFlowHelper {

  /**
   * find all available exchanges assignable to this message. Must be acceptable about functions realized by the source
   * and functions implemented by destination
   * 
   * @param message
   *          the message to affect
   */
   ...
   * @return
   */
  public static List<FunctionalExchange> getAvailableFonctionalExchanges(AbstractInstance sourceInstance,
      AbstractInstance targetInstance) {
    List<FunctionalExchange> result = new ArrayList<FunctionalExchange>();
    List<FunctionalExchange> resultExchangeSender = new ArrayList<FunctionalExchange>();
    List<FunctionalExchange> resultExchangeReceiver = new ArrayList<FunctionalExchange>();

    if (targetInstance != null) {
      List<AbstractFunction> resultReceiver = findFunctionRealizedByAbstractInstance(targetInstance);
      for (AbstractFunction function : resultReceiver) {
        resultExchangeReceiver.addAll(getExchangeDestinationByFunction(function));
      }
    }
    ...
    if (sourceInstance != null) {
      List<AbstractFunction> resultSender = findFunctionRealizedByAbstractInstance(sourceInstance);
      for (AbstractFunction function : resultSender) {
        resultExchangeSender.addAll(getExchangeSourceByFunction(function));
      }
    }
  ...
  public static List<AbstractFunction> findFunctionRealizedByInstanceRole(InstanceRole instanceRole) {
    ...
    if (instanceRole != null) {
      ...
      return findFunctionRealizedByAbstractInstance(ai);
    }
    return result;
  }
   ...
   * @return
   */
  private static boolean isDirectionCompatible(OrientationPortKind portOrientation,
      OrientationPortKind searchedOrientation) {
   ...
   * @return
   */
  public static List<FunctionalExchange> getAvailableFonctionalExchangesFromFunctions(InstanceRole sourceIR,
      InstanceRole targetIR) {
    ...
    return getAvailableFonctionalExchangesFromFunctions(resultSender, resultReceiver);
  }
   ...
   * @return
   */
  public static List<FunctionalExchange> getAvailableFonctionalExchangesFromFunctions(AbstractFunction resultSender,
      AbstractFunction resultReceiver) {
    List<FunctionalExchange> result = new ArrayList<FunctionalExchange>();
   ...
   * @return
   */
  public static Collection<? extends AbstractEventOperation> getAvailableComponentExchanges(InstanceRole sourceIR,
      InstanceRole targetIR) {
    Component sourceComponent = (Component) (sourceIR == null ? null
        : sourceIR.getRepresentedInstance().getAbstractType());
    Component targetComponent = (Component) (targetIR == null ? null
        : targetIR.getRepresentedInstance().getAbstractType());

    return getAvailableComponentExchanges(sourceComponent, targetComponent);
  }
   ...
   * @return
   */
  public static Collection<? extends AbstractEventOperation> getAvailableComponentExchanges(Component sourceComponent,
