 ...
 * (org.polarsys.capella.core.data.interaction.properties.dialogs.sequenceMessage.model.SelectInvokedOperationModelForSharedDataAndEvent)
 * to support searching of available exchanges between the elements (not only between instance roles); Example of
 * refactoring in getAvailableExchangeItems(InstanceRole, InstanceRole) will call getAvailableExchangeItems(AbstractInstance, AbstractInstance)
 */
public class SelectInvokedOperationModelForSharedDataAndEvent {
   ...
   */
  public static List<CapellaElement> getAvailableExchangeItems(InstanceRole source, InstanceRole target,
    ...
    return getAvailableExchangeItems(client, provider, isSynchronous);
  }
   ...
   * @return a list of <code>Operation</code> instances
   */
  public static List<CapellaElement> getAvailableExchangeItems(AbstractInstance client, AbstractInstance provider,
      boolean isSynchronous) {
    ...
    if (client != null) {
      if (client.getAbstractType() instanceof Component) {
        for (Component component : ComponentExt.getAllAncestors((Component) client.getAbstractType())) {
          clientUsedAndRequiredInterfaces
              .addAll(InterfaceExt.getAllSuperGeneralizableElements(getUsedAndRequiredInterfaces(component)));
        }
        ...
        List<EObject> lst = EObjectExt.getReferencers(client.getAbstractType(),
    ...
    if (provider != null) {
      if (provider.getAbstractType() instanceof Component) {
        for (Component component : ComponentExt.getAllAncestors((Component) provider.getAbstractType())) {
          providerImplementedAndProvidedInterfaces
              .addAll(InterfaceExt.getAllSuperGeneralizableElements(getImplementedAndProvidedInterfaces(component)));
        }
      } else if (provider.getAbstractType() instanceof ExchangeItem) {
        // The interfaces are all those containing a ExchangeItemAllocation to this EI
        List<EObject> lst = EObjectExt.getReferencers(provider.getAbstractType(),
            CsPackage.Literals.EXCHANGE_ITEM_ALLOCATION__ALLOCATED_ITEM);
        for (EObject obj : lst) {
          Interface interf = (Interface) obj.eContainer();
